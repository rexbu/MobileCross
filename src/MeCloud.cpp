...
void MeCloud::get(const char* url, MeCallback* callback) {
    char *newUrl = getEncodeUrl((char *)url);
...
void MeCloud::get(const char* url, map<string, string> param, MeCallback* callback) {
    ...
    snprintf(tempUrl, sizeof(tempUrl), "%s", url);
    for (map<string, string>::iterator iter = param.begin(); iter != param.end(); iter++) {
        if (iter == param.begin()) {
            snprintf(tempUrl, sizeof(tempUrl), "%s?%s=%s", tempUrl, iter->first.c_str(), iter->second.c_str());
        } else {
            snprintf(tempUrl, sizeof(tempUrl), "%s&%s=%s", tempUrl, iter->first.c_str(), iter->second.c_str());
        }
    }
    
    char *newUrl = getEncodeUrl((char *)tempUrl);
...
void MeCloud::post(const char* url, const char* body, MeCallback* callback){
    ...
    char *newUrl = getEncodeUrl((char *)url);
    m_http_session.post(newUrl, output, (uint32_t)strlen(output), callback);
    delete newUrl;
    free(output);
}
...
char* MeCloud::getEncodeUrl(char *oldUrl) {
    char *newUrl = new char[URL_SIZE];
    memset(newUrl, 0, URL_SIZE);
    sscanf(oldUrl, "%[^?]", newUrl);
    if (strcmp(newUrl, oldUrl) != 0) {
        char *param = bs_strrstr(oldUrl, "?") + 1;
        char *cryptParam = crypto(param, strlen(param));
        snprintf(newUrl, URL_SIZE, "%s?%s", newUrl, cryptParam);
        free(cryptParam);
    }
    return newUrl;
}
