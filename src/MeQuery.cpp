...
void MeQuery::request(MeCallback* callback) {
    ...
    formatParam();
    snprintf(m_url, sizeof(m_url)-1, "%s%s?%s", MeCloud::shareInstance()->classUrl(), m_classname, m_param);
...
void MeQuery::requestAggregate(MeCallback* callback) {
    ...
    formatParam();
    snprintf(m_url, sizeof(m_url)-1, "%squery/?%s", MeCloud::shareInstance()->restUrl(), m_param);
